...
private[spark] object DecisionTreeMetadata extends Logging {
  ...
  def buildMetadata(
    ...
    if (numClasses > 2) {
      ...
      //多分类
      // split = 2 ^ {M - 1} - 1,bin = 2 * split
      // 根据maxPossibleBins，利用如下公式（是无序公式2 * 2 ^{M-1} - 1的反向推导），来求解m值
      ...
      strategy.categoricalFeaturesInfo.foreach { case (featureIndex, numCategories) =>
        ...
        if (numCategories > 1) {
          ...
          // 2 * ((1 << numCategories - 1) - 1) <= maxBins,若当前的特征数量小于m值，则认为是无序
